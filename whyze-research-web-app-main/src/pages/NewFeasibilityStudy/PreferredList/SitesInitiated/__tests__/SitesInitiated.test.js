...
describe('test cases for SitesContactedTab ', () => {
  test('create snapshots for SitesContactedTab', () => {
    const { container } = renderWithContext(<SitesInitiatedTab />);
    act(() => {
      const { segments, activeSegment } = JSON.parse(
        JSON.stringify(Store.getState().feasibilityStudyReducer),
      );
      segments[activeSegment.index].sitesInitiated.push(demoSites);
  ...
  test('should linked sites expand click & render all linked sites count ', () => {
    ...
    act(() => {
      ...
      segments[activeSegment.index].sitesInitiated.push(demoWithLinkedSites);
  ...
  test('should linked sites expand click & render all linked sites many count ', () => {
    ...
    act(() => {
      ...
      segments[activeSegment.index].sitesInitiated.push(
  ...
  test('should using icon for change initialted to prefferedSites', () => {
    const { container } = renderWithContext(
      <SitesInitiatedTab
        onTabChange={() => {
          'sitesInitiated';
    ...
    screen.debug(preferredListCheckbox[0]);
  });
  test('should using icon for change  initialted to contacted', () => {
    const { container } = renderWithContext(
      <SitesInitiatedTab
        onTabChange={() => {
          'sitesInitiated';
