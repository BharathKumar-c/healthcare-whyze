...
const updateUserTherapeuticAreas = async (req, res) => {
  try {
    const { id } = req.params;
    await preferenceService.updateTherapeuticAreas(req.userProfile.userId, id);

    return res.status(200).send({
      message: updatedTherapeuticAreas,
    });
  } catch (error) {
    return res.status(500).send({ message: error?.message || '' });
  }
