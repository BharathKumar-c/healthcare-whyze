...
const { regex } = require('../../appConstants/regexConstant');
...
const register = async (req, res) => {
  try {
    const { email, password, phone_number, phone_verification_id, country } =
      req.body;
...
const checkForExistingPatient = async (req, res) => {
  ...
  try {
    if (!firstName || !lastName || !dob || !addressLine1) {
      return res.sendStatus(400);
    }
...
const setPassword = async (req, res) => {
  try {
    ...
    const verifiedToken = await jwtService.verifyToken(
      token,
      'forgot_password',
    );
    if (
      verifiedToken.role === role.patient &&
      new Date(new Date(0).setUTCSeconds(verifiedToken.exp)) -
        new Date(Date.now()) >
        0
    ) {
      const data = {
    ...
    return res.status(408).send({ message: linkExpired });
  } catch (error) {
    return res.status(500).send({ message: error?.message || '' });
  }
};
...
const updatePassword = async (req, res) => {
  ...
  try {
    ...
    if (
      verifiedToken.role !== role.patient ||
      Date.now() > new Date(verifiedToken.exp * 1000)
    ) {
      return res.status(408).send({ message: linkExpired });
    }

    if (!regex.password.test(password)) {
      return res.status(408).send({ message: passwordDoNotMatch });
    }

    const user = await patientService.getPatientDetailsByEmail({ email });
    if (!user) {
      return res.status(408).send({ message: passwordDoNotMatch });
    }
...
// Below - Hcp controllers

const hcpLogin = async (req, res) => {
  try {
    const { email, password } = req.body;
...
const setupAccount = async (req, res) => {
  try {
    ...
    const result = await accountService.setupPassword({
      verificationCode,
      email,
      password,
    });
...
const acceptRequest = async (req, res) => {
  try {
    ...
    if (!token) {
      throw new Error(tokenNeed);
    }

    const response = await accountService.acceptLinkRequest(token);
...
const acceptSharingAccess = async (req, res) => {
  try {
    const { token } = req.query;
    ...
    const response = await accountService.acceptSharingAccount(token);
