...
const getHIMasterData = async () => {
  try {
    const query = {
      $or: [{ is_system_added: true }, { is_system_added: { $exists: false } }],
    };
...
const addMasterReactionDetails = async (caseObj, userId) => {
  try {
    return ReactionMaster.create({
      ...caseObj,
      created_by: userId,
      updated_by: userId,
    });
