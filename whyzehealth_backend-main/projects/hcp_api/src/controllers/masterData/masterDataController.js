...
const addMasterReaction = async (req, res) => {
  try {
    if (!req.body.name) {
      return res.status(400).send({ message: emptyReaction });
    }

    await masterService.addMasterReactionDetails(
      req.body,
      req.userProfile.userId,
    );
    return res.status(200).send({ message: reactionAdded });
  } catch (error) {
    return res.status(500).send({ message: error?.message || '' });
  }
};
